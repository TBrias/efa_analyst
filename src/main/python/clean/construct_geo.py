...
def construct_geo_df(df_data):
    ...
    df_data = df_data.withColumn('CP', 
        ...
        .otherwise(when((F.regexp_extract("Rien2", "(\d{5})", 0).cast('int') > 1), F.regexp_extract("Rien2", "(\d{5})", 0))
        .otherwise(when((F.regexp_extract("Immeuble", "(\d{5})", 0).cast('int') > 1), F.regexp_extract("Immeuble", "(\d{5})", 0))
        .otherwise("N/A"))))))
            )
        )
    )
